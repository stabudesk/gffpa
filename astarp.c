...
typedef struct /* opt_t, a struct for the options */
{
	...
	char *rstr; /* blast output format 7 */
} opt_t;
...
typedef struct /* rsta_t: the r option, based on gf22_t */
{
	char *s1; // first seq shortish one, col 3
	size_t s1sz; 
	char *s2; // 2nd seq, longish one, col 5
	size_t s2sz;
	char *mn; // mirna name col 8
	size_t mnsz; /* size of the name r ID field */
} rsta_t; // array star format
...
int catchopts(opt_t *opts, int oargc, char **oargv)
{
	...
	while ((c = getopt (oargc, oargv, "dsni:f:u:r:g:y:a:h:z:b:")) != -1)
		switch (c) {
			case 'd':
				opts->dflg = 1;
				break;
...
rsta_t *processrsta(char *fname, int *m, int *n) /* this is dummy nmae .. it's for the special gff format for ;yze annottion */
{
	/* In order to make no assumptions, the file is treated as lines containing the same amount of words each,
	 * except for lines starting with #, which are ignored (i.e. comments). These words are checked to make sure they contain only floating number-type
	 * characters [0123456789+-.] only, one string variable is continually written over and copied into a growing floating point array each time */
	...
	rsta_t *rsta=malloc(GBUF*sizeof(rsta_t));
	...
	while( (c=fgetc(fp)) != EOF) { /* grab a char */
		...
		if( (c== '\n') | (c == '\t') | (c=='#')) { /* when dealing with strict tab spearated froamts like gff3, can we exclude space as a separator? */
			if( inword==1) { /* first word closing event */
				...
				bufword = realloc(bufword, couc*sizeof(char)); /* normalize */
				/* for the struct, we want to know if it's the first word in a line, like so: */
				if((couw-oldcouw)==3) { /* fourth col */
					rsta[wa->numl].s1=malloc(couc*sizeof(char));
					rsta[wa->numl].s1sz=couc;
					strcpy(rsta[wa->numl].s1, bufword);
				} else if((couw-oldcouw)==5) { /* fourth col */
					rsta[wa->numl].s2=malloc(couc*sizeof(char));
					rsta[wa->numl].s2sz=couc;
					strcpy(rsta[wa->numl].s2, bufword);
				} else if((couw-oldcouw)==8) { /* fourth col */
					rsta[wa->numl].mn=malloc(couc*sizeof(char));
					rsta[wa->numl].mnsz=couc;
					strcpy(rsta[wa->numl].mn, bufword);
				}
				couc=0;
				couw++;
			}
			if(c=='#') { /* comment case */
				...
				if(wa->numl == wa->lbuf-1) { /* enought space in our current array? */
					wa->lbuf += WBUF;
					wa->wpla=realloc(wa->wpla, wa->lbuf*sizeof(size_t));
					rsta=realloc(rsta, wa->lbuf*sizeof(rsta_t));
					memset(wa->wpla+(wa->lbuf-WBUF), 0, WBUF*sizeof(size_t));
				}
				wa->wpla[wa->numl] = couw-oldcouw; /* number of words in current line */
				/* extra bit to catch canonical colnums */
			...
			if(couw == wa->wsbuf-1) {
				wa->wsbuf += GBUF;
				wa->wln=realloc(wa->wln, wa->wsbuf*sizeof(size_t));
				rsta=realloc(rsta, wa->wsbuf*sizeof(rsta_t));
				for(i=wa->wsbuf-GBUF;i<wa->wsbuf;++i)
					wa->wln[i]=0;
			}
			couc=0;
	...
	wa->quan=couw;
	wa->wln = realloc(wa->wln, wa->quan*sizeof(size_t)); /* normalize */
	rsta = realloc(rsta, wa->quan*sizeof(rsta_t)); /* normalize */
	wa->wpla= realloc(wa->wpla, wa->numl*sizeof(size_t));
...
void prtrsta(char *fname, rsta_t *rsta, int m6)
{
	int i;
...
void prtusage()
{
	printf("astarp: a simple parser for Array Star's novoalign output format.\n");
	printf("0ptions:\n");
	...
	printf("-r: takes the array star file, eg. Parr1_Salmo_salar_ssa_novoalign_premiRNA.txt\n");
	return;
}
...
int main(int argc, char *argv[])
{
	...
	int i, m, n /*rows,cols for bgr_t*/, m2, n2 /*bed2 (gff conv to bed) dims */, m3, n3, m5, n5, n6, m6, m7, n7 /*gf22 dims */, m8, n8 /* gf23 dims */, mb, nb /*blast out file */;
	...
	if(opts.rstr)
		rsta=processrsta(opts.rstr, &m6, &n6);
	if(opts.gstr)
